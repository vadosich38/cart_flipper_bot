...
async def next_pair_callback(callback_data: CallbackQuery, state: FSMContext) -> None:
    await callback_data.answer(text="Следующая карточка ➡️")
    data = await state.get_data()
    cur_card = data["spec_coll_pag_inst"].next()
